...
class HDF5Dataset(data.Dataset):
            ...
            for gname, group in h5_file.items():
                ...
                for dname, ds in group.items():
                    ...
                    self.data_info.append(
                        {'file_path': file_path, 'type': dname, 'shape': ds[()].shape, 'cache_idx': idx, 'label': label})
    ...
    def _load_data(self, file_path):
        ...
        if len(self.data_cache) > self.data_cache_size:
            ...
            self.data_info = [
                {'file_path': di['file_path'], 'type': di['type'], 'shape': di['shape'], 'cache_idx': -1, 'label': label} if
                di['file_path'] == removal_keys[0] else di for di in self.data_info]

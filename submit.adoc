...
6. Log File requirements.
   ...
   1. The log file must have am Organization Records in Mllog entry format. This includes mllog entries for `POINT_IN_TIME` with the values
      ...
      * submission_platform
      ...
      The submission division such as closed and open must be elected in the "submission_division" filed. The benchmarks for the closed divisioon 
      are defined by the codes cloudmahs, earthquake, stemdl, and uno contained in the science benchmark repository. The version in github 
      used for the benchmark needs to be added to the submission log record. 

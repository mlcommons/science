...
`{descriptor}` ::= The descriptor of the experiment, as described in the sientific_contribution. For example `experiment-1`, `faultzone-3`.
...
5. A README file . A README.md file that describes how to run it. 
...
Each benchmark must have an mllog entry  POINT_IN_TIME with the key "result" and the value of a dict describint the result. 
